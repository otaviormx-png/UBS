const routes = [
    ...
    patients.forEach(p => {
      markers[p.id]=L.marker([p.lat,p.lng],{icon:icon(p)}).addTo(map).bindPopup(`<b>${p.name}</b><br>${p.reason}<br><small>${p.address} | Microárea ${p.area}</small>`);
    ...
    function addManualCase(){
      ...
      patients.push(newCase);
      markers[id]=L.marker([newCase.lat,newCase.lng],{icon:icon(newCase)}).addTo(map).bindPopup(`<b>${newCase.name}</b><br>${newCase.reason}<br><small>${newCase.address} | Aguardando localização</small>`);
      markers[id].on("click",()=>selectPatient(id,false));
      [newName,newBirth,newDoc,newPhones,newAddress,newVisitDate,newProfessional,newProblem,newAssist].forEach(field => field.value = "");
      intakeNote.textContent = "Caso adicionado. Use a conferência de endereço para validar a área da UBS.";
      selectedId = id;
      render();
      selectPatient(id,false);
    }
    ...
    function updateVisitDone(id,value){
      const p=patients.find(item=>item.id===id);
      if(!p) return;
      p.visitDone=value;
      render();
